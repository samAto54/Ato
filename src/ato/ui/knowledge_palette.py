...
KNOWLEDGE_ACTIONS = (
    ("import", "IMPORT DOCUMENT", "Index one workspace file", "HIGH"),
    ("refresh", "REFRESH INDEX", "Reload document metadata", "LOW"),
    ("remove", "REMOVE DOCUMENT", "Delete indexed excerpts by ID", "HIGH"),
)


class KnowledgeActionPalette:
    """Modal palette exposing only fixed Knowledge operations."""
    ...
    def __init__(self, parent, theme: UiTheme, on_select: Callable[[str], None]) -> None:
        ...
        self.window.title("Ato Knowledge Controls")
        ...
        tk.Label(
            panel,
            text="Local bounded RAG index",
            bg=theme.panel,
            fg=theme.muted_text,
            font=(theme.font_family, 9),
            anchor="w",
        ).pack(fill="x", pady=(2, 10))
        for name, label, description, risk in KNOWLEDGE_ACTIONS:
            tk.Button(
                panel,
                text=f"{label:<20} {risk}\n{description}",
